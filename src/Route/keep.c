...
// double speed_last = 0;			//保存上一次的速度，计算加速度
double speed_last_x = 0;
double speed_last_y = 0;
/*
 * 函数名: SetKeep
 * 描  述: 快速到达指定位置与角度
 * 输  入: -end_point  目标位置
...
void Keep(void)
{
		...
		if(Speed_X>=1200)
			Speed_X=1200;
		else if(Speed_X<=-1200)
			Speed_X=-1200;
		if(fabs(Speed_Y>=1200))
			Speed_Y=1200;
		else if(Speed_Y<=-1200)
			Speed_Y=-1200;
		//SetSpeed(Speed_X,Speed_X
		SetSpeed2(Speed_X,Speed_Y,0);
...
// 		
// 		LCD_SetXY(0,2);
// 		LCD_WriteString("Destination_X:");
// 		LCD_WriteDouble(Aim_Keep_Point.x);
// 		
// 		LCD_SetXY(0,3);
// 		LCD_WriteString("Destination_Y:");
// 		LCD_WriteDouble(Aim_Keep_Point.y);
